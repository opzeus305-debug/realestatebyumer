...
- **The twitch has three causes, in order of contribution.**
  1. **Z-fighting across the whole facade (primary — confirmed).** The material-ID render (`?flat=1&offset=0`) shows "Vitres" covering nearly the *entire visible tower*: it is 1 % of the triangles but ~90 % of the surface area — an outer glass envelope lying on the same planes as the "Rebord" mullion lattice. With the glass opaque, the depth test decides per pixel, per frame, whether mullion or glass wins, and the winner changes with every sub-pixel camera move. That is a moving stipple over the whole building, i.e. the "twitch". The near plane at 2.2 reduced the precision loss but cannot resolve truly coplanar faces. Fix: `polygonOffset` on the glass (factor 1, units 2) so the mullions always win — deterministic, free.
  2. **Specular aliasing (secondary).** 661k triangles of sub-pixel mullions at roughness ≈0.3 under an HDR sky and hard point lights: each frame a different sub-pixel set catches the highlight, so the lattice sparkles as the camera drifts. MSAA cannot fix this — it is shading aliasing, not edge aliasing — and it is worst at DPR 1.0. Confirm with `?saa=0` vs default.
  3. **The water normal map (tertiary).** A normal map that scrolls every frame perturbs the sky reflection under the tower; the tower's reflection wobbles and reads as twitch by association. The production ripple is too high-frequency and too fast for a lake.
